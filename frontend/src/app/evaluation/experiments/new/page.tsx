  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      ...
      <div className="space-y-4 rounded-xl border bg-white p-5">
        ...
        <div className="rounded-lg bg-amber-50 p-3 text-xs text-amber-800">预计执行 {estimateCaseRuns(selected?.cases.length || 0, repetitions)} 次用例。评测消息走生产 PuddingHarness 执行框架；业务工具与 MCP 默认禁用。{isSWEbench ? "平台先准备与官方测试规格一致的智能体依赖环境，智能体产出补丁后再启动干净的官方 Docker 判卷器。首次运行需要拉取或构建镜像，耗时和磁盘占用明显高于普通评测。" : selected?.default_profile === "coding_agent@1" ? "代码题使用生产执行框架的文件与执行协议，评分以可信隐藏验证结果为准。" : "工作区内建文件工具仍可用于固定数据场景。"}</div>
