...
function newCase(): EvalCase {
  ...
  return {
    ...
    name: "新 Case", description: "", enabled: true, repetitions: 1, dimensions: [],
    input: { message: "", turns: [] },
...
export default function DatasetDetailPage() {
  ...
  const choose = (next: EvalCase) => {
    if (dirty && !window.confirm("当前 Case 有未保存修改，确定放弃吗？")) return;
  ...
  const precheck = async () => {
    if (dirty) { setError("请先保存当前 Case，再执行预检查。"); return; }
    setBusy(true);
  ...
  const publish = async () => {
    if (!dataset) return;
    if (dirty) { setError("请先保存当前 Case，再发布版本。"); return; }
    setBusy(true);
  ...
  const remove = async () => {
    ...
    if (!window.confirm("删除这个 Case？")) return;
    setBusy(true);
  ...
  return <div className="grid min-h-full grid-cols-[280px_minmax(0,1fr)]">
    ...
    <section className="p-6">{error && <div className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div>}
      ...
      {selected && <><div className="mb-3 max-w-4xl rounded-lg bg-blue-50 px-4 py-3 text-xs text-blue-800">{selected.code ? "填写任务说明、初始源码、隐藏测试和验证命令即可；Agent 只能看到初始源码，评分只认隔离验证结果。" : "最少只需要填写“名称、输入、回答必须包含”三项；其余字段可以保持默认。"}</div><div className="grid max-w-4xl grid-cols-2 gap-4 rounded-xl border bg-white p-5">
