...
def _latest_artifact_versions(
    ...
    for ref in refs:
        path = str(ref.get("host_path") or ref.get("path") or ref.get("virtual_path") or ref.get("artifact_id") or "")
        if not path:
            continue
        current = latest.get(path)
        timestamp = float(ref.get("written_at") or ref.get("_activation_created_at") or 0)
        current_timestamp = float(
            (current or {}).get("written_at") or (current or {}).get("_activation_created_at") or 0
        )
...
# Suffixes with a registered executable validator (syntax/static check, test
# runner, HTML/browser validation, or JSON structure check).  Prose and data
# artifacts (.md notes, evidence files, …) have nothing to execute: their
# correctness is judged by the LLM-graded criteria, so they must not enter the
# code-validation acceptance set — otherwise no validator could ever close the
# criterion and the Goal deadlocks.
_EXECUTABLE_VALIDATION_SUFFIXES = frozenset(
    {
        ".html",
        ".htm",
        ".js",
...
def _requires_executable_validation(item: dict[str, Any]) -> bool:
    raw = str(item.get("path") or item.get("host_path") or item.get("virtual_path") or "")
    suffix = Path(raw).suffix.lower()
    if not suffix:
        # No path to classify the artifact by: fail closed and keep requiring
        # executable validation evidence.
        return True
    return suffix in _EXECUTABLE_VALIDATION_SUFFIXES
...
def _artifact_acceptance_set(
    ...
    *,
    ...
    if not any(str(item.get("role") or "") for item in latest):
        ...
        return latest
    targets = [item for item in latest if str(item.get("role") or "") == ArtifactRole.TARGET.value]
    if targets:
        return targets
    current_candidates = [
        item
        for item in latest
        if str(item.get("run_id") or "") == run_id and str(item.get("role") or "") != ArtifactRole.TEMPORARY.value
    ]
    deliverable = [item for item in latest if str(item.get("role") or "") != ArtifactRole.TEMPORARY.value]
...
def evaluate_deterministic_criteria(
    ...
    harness_context["browser_e2e_required"] = bool(contract.browser_e2e_required)
        ...
        if criterion.id == "todo_reconciliation":
            ...
            evaluations.append(
                CriterionEvaluation(
                    ...
                    gap=("该规则声明为 deterministic，但没有注册对应的代码验证器；Harness 按 fail-closed 处理。"),
                )
            )
    return evaluations
...
def _evaluate_analytics_model_invariants(
    ...
    model_id = str(final_state.get("analytics_model_id") or harness_context.get("analytics_model_id") or "").strip()
    if not model_id:
...
def _evaluate_todos(
    ...
    todos = [
        ...
        and (
            ...
            or (not current_goal_id and str(item.get("created_run_id") or current_run_id) == current_run_id)
        )
    ]
    incomplete = [
        item
        for item in todos
        if isinstance(item, dict) and str(item.get("status") or "").lower() not in {"completed", "cancelled"}
    ]
    evidence = [
        {
            "kind": "todo_state",
    ...
    labels = [
        str(item.get("content") or item.get("title") or item.get("id") or "未命名 Todo") for item in incomplete[:5]
    ]
    return CriterionEvaluation(
        criterion_id=criterion_id,
        name=criterion_id,
...
def _evaluate_artifact_delivery(
    ...
    activations = [item for item in _verification_activations(harness_context, {}) if item.get("pack") == "artifact"]
    current_refs = [
        ref
    ...
    mutation_records = [
        ...
        for activation in activations
        for ref in activation.get("evidence_refs") or []
        if isinstance(ref, dict) and ref.get("kind") == "external_mutation_completed"
    ]
    ...
    mutations_by_id = {str(item.get("receipt_id") or ""): item for item in mutation_records if item.get("receipt_id")}
    current_goal_id = str(harness_context.get("goal_id") or "")
    current_goal_revision = harness_context.get("goal_revision")
    current_workspace_id = str(harness_context.get("workspace_id") or "")
    active_grant_ids = {str(item) for item in (harness_context.get("active_permission_grant_ids") or []) if item}
    ...
    declared_targets = [str(item) for item in (harness_context.get("declared_artifact_targets") or []) if item]
    uncovered_targets = [
        declared
    ...
    for ref in selected_refs:
        ...
        if parsed.scope == ArtifactScope.EXTERNAL:
            mutation = mutations_by_id.get(parsed.mutation_receipt_id) if parsed.mutation_receipt_id else None
            mutation_matches = bool(
                isinstance(mutation, dict)
                and str(mutation.get("canonical_path") or "") == str(parsed.host_path or parsed.path)
                and str(mutation.get("after_sha256") or "") == str(parsed.content_sha256 or "")
                and str(mutation.get("permission_grant_id") or "") == str(parsed.permission_grant_id or "")
                and str(mutation.get("run_id") or "") == str(parsed.run_id or "")
            )
            ...
            declared_authority = bool(
                ...
                and parsed.permission_grant_id == f"declared-artifact:{parsed.run_id}"
                and mutation_matches
                and declared_target_matches
            )
            legacy_authority = bool(
                parsed.authorized
                and parsed.authority_kind == "legacy_declared_artifact_backfill"
                and str(parsed.mutation_receipt_id or "").startswith("legacy-write-backfill:")
                and parsed.permission_grant_id == f"declared-artifact:{parsed.run_id}"
                and declared_target_matches
            )
            persistent_grant_authority = bool(
                ...
                and (not grants_authoritative or parsed.permission_grant_id in active_grant_ids)
            )
            if not (persistent_grant_authority or declared_authority or legacy_authority):
                invalid.append(
                    {
                ...
                continue
        if current_workspace_id and parsed.workspace_id and parsed.workspace_id != current_workspace_id:
            invalid.append(
                {
                    "artifact_id": parsed.artifact_id,
                    "path": parsed.path,
        ...
        if not artifact_path.is_file():
            ...
            continue
        if parsed.receipt_version >= 2 and (not parsed.content_sha256 or parsed.size_bytes is None):
            invalid.append(
                {
                    "artifact_id": parsed.artifact_id,
                    "path": parsed.path,
        ...
        if enforce_publication_reference and (
            not final_content or not any(value in final_content for value in reference_candidates)
        ):
            unreferenced.append(
                {
                    "artifact_id": parsed.artifact_id,
    ...
    if uncovered_targets:
        gap = "尚未写入用户明确指定的交付目标：" + "；".join(uncovered_targets[:5]) + "。workspace 副本不能替代该目标。"
...
def _verification_activations(
    ...
    if not isinstance(raw, list):
        return []
    return [item for item in raw if isinstance(item, dict) and item.get("status") == "succeeded"]
...
def _evaluate_evidence_traceability(
    ...
    activations = [
        item for item in _verification_activations(harness_context, final_state) if item.get("pack") == required_pack
    ]
    evidence = [
        evidence_ref
        for activation in activations
    ...
    if required_pack == "web_research":
        ...
        evidence_urls = {str(item.get("uri") or "").rstrip("}),]") for item in sources if item.get("uri")}
        evidence_source_ids = {str(item.get("source_id") or "") for item in sources if item.get("source_id")}
        cited_source_ids = {match.group(0)[2:-1] for match in _SOURCE_CITATION_PATTERN.finditer(final_content)}
        ...
        source_call_ids = {
            str(source.get("tool_call_id") or "") for source in unique_sources.values() if source.get("tool_call_id")
        }
        ...
        if not sources or not (explicitly_cited or unambiguous_tool_lineage):
            return CriterionEvaluation(
                ...
                verifier=VerifierKind.DETERMINISTIC,
                evidence=evidence,
                gap=("当前 Goal 修订版没有形成可验证的来源链路，或多来源回答没有显式引用真实 source_id / 网页链接。"),
            )
    if required_pack == "analytics":
        result_evidence = [
            item
            for item in evidence
            if item.get("kind") in {"tool_result", "analytics_result"} and item.get("tool_call_id")
        ]
        if not result_evidence:
            return CriterionEvaluation(
                ...
                evidence=evidence,
                gap=(
                    "分析 pack 已激活，但缺少当前 Goal 修订版查询结果的输出摘要、result_id、query trace 或数据源引用。"
                ),
            )
    ...
    return CriterionEvaluation(
        ...
        verifier=VerifierKind.DETERMINISTIC,
        evidence=evidence,
        gap=(f"缺少当前 Goal 修订版的结构化 {pack_label} 工具证据；不能仅凭模型声称“有来源”判定可追溯。"),
    )
...
def _evaluate_code_validation(
    ...
    activations = [
        ...
        if isinstance(item, dict) and item.get("pack") == "code" and item.get("status") in {"succeeded", "failed"}
    ]
    current_write_refs = [
        {
            **evidence_ref,
            "_source_current": True,
            "_activation_created_at": float(activation.get("created_at") or 0),
        }
        for activation in activations
        for evidence_ref in activation.get("evidence_refs") or []
        if isinstance(evidence_ref, dict)
    ...
    inherited_write_refs = [
        {
            **item,
            ...
            "_activation_created_at": float(item.get("written_at") or item.get("created_at") or 0),
        }
    ...
    invalid_inherited_writes = [
        ...
        if item.get("_source_inherited") and not _artifact_evidence_matches_current_bytes(item, harness_context)
    ]
    acceptance_writes = [
        item
        for item in acceptance_candidates
        if not item.get("_source_inherited") or _artifact_evidence_matches_current_bytes(item, harness_context)
    ]
    validation_scope_writes = [item for item in acceptance_writes if _requires_executable_validation(item)]
    acceptance_identity = {
        artifact_identity(item)
        for item in validation_scope_writes
    ...
    latest_write_at = max(
        (float(item.get("written_at") or item.get("_activation_created_at") or 0) for item in acceptance_writes),
        default=0.0,
    )
    raw_receipts = [
        {
    ...
    raw_receipts.extend(
        ...
        for item in inherited
        if item.get("kind") == "validation_receipt" and item.get("verification_pack") == "code"
    )
    acceptance_write_timestamps = {
        artifact_identity(item): float(item.get("written_at") or item.get("_activation_created_at") or 0)
        for item in validation_scope_writes
        if (item.get("path") or item.get("artifact_id")) and item.get("content_sha256")
    }
    ...
    def _attempted_acceptance_identities(
        ...
        for item in validation_scope_writes:
            aliases = {
                str(item.get(field) or "").strip() for field in ("path", "host_path", "virtual_path") if item.get(field)
            }
    ...
    def _attempt_is_current(activation: dict[str, Any]) -> bool:
        ...
        if identities:
            return any(created_at >= acceptance_write_timestamps.get(identity, 0) for identity in identities)
    ...
    for activation in activations:
        ...
        unreceipted_successful_attempts.extend(
            {
                **execution,
                "activation_created_at": float(activation.get("created_at") or 0),
                "_attempted_artifact_identities": [list(identity) for identity in sorted(attempted_identities)],
            }
    ...
    for receipt in raw_receipts:
        bound_refs = [
            ...
            if isinstance(item, dict) and (item.get("path") or item.get("artifact_id")) and item.get("content_sha256")
        ]
        bound_identities = {artifact_identity(item) for item in bound_refs}
        if bound_identities:
            coverage = bound_identities & acceptance_identity
            extra_refs = [item for item in bound_refs if artifact_identity(item) not in acceptance_identity]
            if coverage and all(_artifact_evidence_matches_current_bytes(item, harness_context) for item in extra_refs):
                relevant_receipts.append(receipt)
            continue
        if receipt.get("_source_current") and float(receipt.get("_activation_created_at") or 0) >= latest_write_at:
            relevant_receipts.append(receipt)
    ...
    artifact_failure_receipts = [
        ...
        if str(receipt.get("failure_class") or "artifact_failure") == "artifact_failure"
    ...
    control_failure_receipts = [
        ...
        for receipt in failed_receipts
        if (
            str(receipt.get("failure_class") or "") in {"invocation_failure", "infrastructure_failure"}
            or (
                str(receipt.get("failure_class") or "artifact_failure") == "artifact_failure"
    ...
    def _same_obligation(
        ...
        failure_key = str(
            failure.get("obligation_key") or f"{failure.get('validator_kind')}:{failure.get('validator_version')}"
        )
        success_key = str(
            success.get("obligation_key") or f"{success.get('validator_kind')}:{success.get('validator_version')}"
        )
        if failure_key != success_key:
            return False
        failure_identities = {
            artifact_identity(item) for item in failure.get("artifact_refs") or [] if isinstance(item, dict)
        }
        success_identities = {
            artifact_identity(item) for item in success.get("artifact_refs") or [] if isinstance(item, dict)
        }
        return (not failure_identities or failure_identities.issubset(success_identities)) and float(
            success.get("created_at") or 0
        ) >= float(failure.get("created_at") or 0)

    unsuperseded_control_failures = [
        failure
        for failure in control_failure_receipts
        if not any(_same_obligation(failure, success) for success in passed_receipts)
    ]
    ...
    receipt_identities = {
        ...
        for item in receipt.get("artifact_refs") or []
        if isinstance(item, dict) and _validator_matches_artifact(receipt, item)
    }
    browser_validated_identities = {
        artifact_identity(item)
        for receipt in passed_receipts
        if receipt.get("validator_kind") in {"browser_runtime", "artifact_ui_contract"}
        for item in receipt.get("artifact_refs") or []
        if isinstance(item, dict)
    }
    html_identity = {
    ...
    if invalid_inherited_writes:
        return CriterionEvaluation(
            ...
            gap=(
                f"最后一次成功验证之后产物又被写入 {len(invalid_inherited_writes)} 次，"
                "内容 hash 已变化，前序验证证据随之失效。"
                "请先完成全部修改并冻结产物，再发起最终验证；验证通过后不要再改动产物。"
            ),
        )
    if uncovered_declared:
        return CriterionEvaluation(
            ...
            gap=("尚未形成全部声明目标的代码产物证据：" + "；".join(uncovered_declared)),
        )
    if acceptance_writes and not missing_validation and (not browser_e2e_required or not missing_browser):
        return CriterionEvaluation(
            criterion_id=criterion_id,
            name=criterion_id,
            passed=True,
    ...
    unreceipted_missing_attempts = [
        ...
        or any(tuple(identity) in missing_validation for identity in item.get("_attempted_artifact_identities") or [])
    ]
    ...
    if acceptance_writes and unsuperseded_control_failures:
        has_infrastructure_failure = any(
            str(item.get("failure_class") or "") == "infrastructure_failure" for item in unsuperseded_control_failures
        )
        ...
        if repeated_invocations:
            repeated_kind, repeated_count = max(repeated_invocations.items(), key=lambda entry: entry[1])
        ...
        return CriterionEvaluation(
            ...
            gap=(
                ...
                else "验收调用参数、路径映射或内容读取证明无效；请修复验证调用，不要修改业务产物。"
            ),
            failure_kind=(
    ...
    if acceptance_writes and validation_attempts and not relevant_receipts:
        return CriterionEvaluation(
            ...
            gap=("校验尝试没有形成可判定的 ValidationReceipt。这是验证协议错误，不应继续修改业务产物。"),
            failure_kind=VerificationFailureKind.VALIDATOR_PROTOCOL_ERROR,
        )
    if acceptance_writes:
        if browser_e2e_required and missing_browser:
            ...
            uncovered = sorted({str(identity[0]) for identity in missing_validation if identity[0]})
            gap = (
                "以下产物缺少与当前 hash 绑定的成功测试、构建或静态检查："
                + "；".join(uncovered)
                + "。请运行与产物匹配的可执行验证（pytest/ruff、npm run build、"
                "node --check <file>，或对 Python 产物运行 "
                "python3 -m py_compile <file>），并保持产物 hash 不再变化。"
            )
    else:
        gap = (
...
def _artifact_evidence_matches_current_bytes(
    ...
    if not raw_path and ref.get("workspace_relative_path"):
        workspace_raw = str(harness_context.get("workspace_path") or "").strip()
        if workspace_raw:
            raw_path = str(Path(workspace_raw).expanduser().resolve() / str(ref.get("workspace_relative_path")))
    if not raw_path:
        return False
