...
suspension barrier, a PREPARED installation migration manifest, and a private
rollback evidence file produced out-of-band by the Knowledge reverse chain
(this orchestrator binds it by digest only).  The manifest first advances
PREPARED to ROLLED_BACK bound to the evidence digest, then the Harness rev2
rollback assignment commits (writer puddingclaw), then the delegated Knowledge
CLI commits its rev2 rollback assignment (both domains puddingclaw), and both
freeze markers are verified to still be in place.  A crash anywhere leaves both
products frozen and fail-closed; exact retry resumes from the last committed
checkpoint and completed checkpoints never downgrade.  Knowledge commands
execute through an explicitly selected independent interpreter.  This command
never thaws: both new products' freeze markers must remain, the legacy source
Home's thaw is the source product's own responsibility, and ROLLED_BACK to
FINALIZED stays fail-closed in the manifest layer.  It does not activate
production and performs no cutover.
"""
...
def _rolled_back_event(journal, side):
    """Pin the assigned revision 2 rollback head of a writer journal."""
    events = journal.get('events') if isinstance(journal, dict) else None
    if not isinstance(events, list) or len(events) != 3:
        raise ValueError('Rollback journal does not contain exactly revisions 0-2')
    ...
    if head.get('revision') != 2 or head.get('state') != 'assigned':
        raise ValueError('Rollback journal head is not an assigned revision 2')
    if side == 'harness':
