"""Black-box runtime contracts for this repository's installed backend.

Run with uv sync --locked --no-editable, then uv run --no-sync pytest.
Each child executes outside the checkout and rejects editable/source imports.
"""
import os
import subprocess
import sys
...
def run_target(code, tmp_path):
    ...
    bootstrap = "import config; assert 'site-packages' in config.__file__, config.__file__\n"
    result = subprocess.run([sys.executable, '-c', bootstrap + code],
